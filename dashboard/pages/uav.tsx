...
const statusLabel = (t: any): Record<string, string> => ({
    active: t('uav.status_active'), idle: t('uav.status_idle'), charging: t('uav.status_charging'), maintenance: t('uav.status_maintenance')
})
...
export default function UAVPage() {
    ...
    const flightTypeLabelT = (type: string) => {
        if (type === 'surveillance') return t('uav.flight_surveillance')
        if (type === 'spraying') return t('uav.flight_spraying')
        if (type === 'mapping') return t('uav.flight_mapping')
        return type
    }
    ...
    return (
        <>
            <Head><title>UAgriAIV – {t('uav.page_title')}</title></Head>
            <Layout title={t('uav.page_title')} subtitle={t('uav.page_subtitle')} alerts={mockAlerts} onRefresh={() => { }}>
                        ...
                        <button key={u.id}
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <div>
                                    ...
                                    <div className="text-xs mt-0.5 flex items-center gap-1.5">
                                        ...
                                        <span style={{ color: statusColor[u.status] }}>{statusLabel(t)[u.status]}</span>
                                    </div>
                                </div>
                            ...
                            <div className="mb-2">
                                <div className="flex items-center justify-between text-xs mb-1">
                                    <span style={{ color: '#7aad8e' }}>{t('common.battery')}</span>
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs mt-3">
                                <div>
                                    <div style={{ color: '#4d7360' }}>{t('common.area')}</div>
                                    <div className="font-semibold text-white">{u.total_area_scanned.toLocaleString()} {t('common.ha')}</div>
                                </div>
                                <div>
                                    <div style={{ color: '#4d7360' }}>{t('uav.remaining')}</div>
                                    <div className="font-semibold text-white">{u.remaining_flight_time} {t('uav.unit_min')}</div>
                                </div>
                            </div>
                        </button>
                    ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="glass-card p-5">
                        <h3 className="font-semibold text-white text-sm mb-4">{uav.name} – {t('uav.details')}</h3>
                        <div className="space-y-4">
                            {[
                                { icon: '🔋', label: t('common.battery'), value: `${uav.battery}%`, color: batteryColor },
                                { icon: '⏱️', label: t('uav.remaining_flight_time'), value: `${uav.remaining_flight_time} ${t('uav.unit_min')}`, color: '#38bdf8' },
                                { icon: '⚡', label: t('uav.flight_speed'), value: `${uav.current_speed} km/h`, color: '#a78bfa' },
                                { icon: '📈', label: t('uav.scan_speed'), value: `${uav.scan_speed} ha/h`, color: '#00d66a' },
                                { icon: '🗺️', label: t('uav.total_area_scanned'), value: `${uav.total_area_scanned.toLocaleString()} ha`, color: '#fbbf24' },
                                {
                                    icon: '📅', label: t('uav.last_flight'),
                    ...
                    <div className="lg:col-span-2 glass-card p-5">
                        <h3 className="font-semibold text-white text-sm mb-1">{t('uav.area_scanned')}</h3>
                        <p className="text-xs mb-4" style={{ color: '#4d7360' }}>{t('uav.area_scanned_sub')}</p>
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={areaHistory} barCategoryGap="35%">
                                ...
                                <Bar dataKey="area" name={t('common.area')} fill="#00d66a" radius={[6, 6, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="glass-card p-5">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h3 className="font-semibold text-white text-sm">{t('uav.flight_history')}</h3>
                            <p className="text-xs mt-0.5" style={{ color: '#4d7360' }}>
                                {uav.name} – {uavFlights.filter(f => f.status !== 'scheduled').length} {t('uav.completed_flights')}
                            </p>
                        </div>
                    </div>

                    {uavFlights.length === 0 ? (
                        <div className="text-center py-8 text-sm" style={{ color: '#4d7360' }}>{t('uav.no_history')}</div>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead>
                                    <tr style={{ borderBottom: '1px solid #1e3528' }}>
                                        {[t('uav.history_date'), t('uav.history_type'), t('uav.history_duration'), t('uav.history_area'), t('uav.history_speed'), t('uav.history_status')].map(h => (
                                        ...
                                        <tr key={f.id} className="border-b hover:bg-white/3 transition-colors"
                                            ...
                                            <td className="py-3 pr-4">
                                                <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                                                    ...
                                                    {flightTypeLabelT(f.flight_type)}
                                                </span>
                                            </td>
                                            <td className="py-3 pr-4 text-white">{f.duration > 0 ? `${f.duration} ${t('uav.unit_min')}` : '—'}</td>
                                            ...
                                            <td className="py-3">
                                                {f.status === 'completed' && (
                                                    <span className="flex items-center gap-1 text-xs" style={{ color: '#34d399' }}>
                                                        <span style={{ fontSize: '10px' }}>✓</span>{t('uav.status_completed')}
                                                    </span>
                                                )}
                                                {f.status === 'aborted' && (
                                                    <span className="flex items-center gap-1 text-xs" style={{ color: '#ef4444' }}>
                                                        <span style={{ fontSize: '10px' }}>✗</span>{t('uav.status_aborted')}
                                                    </span>
                                                )}
                                                {f.status === 'scheduled' && (
                                                    <span className="flex items-center gap-1 text-xs" style={{ color: '#fbbf24' }}>
                                                        <span style={{ fontSize: '10px' }}>📅</span>{t('uav.status_scheduled')}
                                                    </span>
                                                )}
                                            </td>
                                        </tr>
