...
export default function MapScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const [heatmapData, alertData] = await Promise.all([
        fetchHeatmap(),
        fetchRecentAlerts(),
      ]);
  ...
  useEffect(() => {
    load();
    const interval = setInterval(load, 15_000); // refresh mỗi 15s
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Markers cho từng điểm phát hiện */}
        {alerts.map(a => (
          <Marker
            key={a.id}
            ...
            description={`${a.severity.toUpperCase()} · ${new Date(a.timestamp).toLocaleTimeString('vi-VN')}`}
          />
        ))}
      </MapView>
      ...
      <View style={styles.alertPanel}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            keyExtractor={i => i.id}
            renderItem={({ item }) => (
              <View style={[styles.alertItem, { borderLeftColor: SEVERITY_COLOR[item.severity] }]}>
...
const styles = StyleSheet.create({
  container:     { flex: 1 },
  map:           { flex: 1 },
  alertPanel:    { maxHeight: 220, backgroundColor: '#0f1a14', paddingHorizontal: 16, paddingTop: 10 },
  panelTitle:    { color: '#2dff7a', fontWeight: '700', marginBottom: 8, fontSize: 13, letterSpacing: 1 },
